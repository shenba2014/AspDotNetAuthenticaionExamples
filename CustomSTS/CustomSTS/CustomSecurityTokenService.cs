...
namespace CustomSTS
{
	internal class CustomSecurityTokenService : SecurityTokenService
	{
		...
		static void ValidateAppliesTo(EndpointReference appliesTo)
		{
			...
			if (!validAppliesTo)
			{
				throw new InvalidRequestException($"The 'appliesTo' address '{appliesTo.Uri.OriginalString}' is not valid.");
			}
		}
		...
		public static X509Certificate2 GetCertificate(string subjectName)
		{
			...
			try
			{
				...
				if (certs.Count == 0)
					...
					throw new ApplicationException($"There are multiple certificates for subject Name {subjectName}");
